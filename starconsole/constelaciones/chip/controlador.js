...
export class `+ _.capitalize(componente.nombre) + `Component implements OnInit {
	...
	@Input() id: number;
    @Input() servicio = {
        ligar`+ planeta.singular + ` : (x, y) => {
            return new Promise(resolve => resolve())
        },
        desligar`+ planeta.singular +  `: (x, y) => {
            return new Promise(resolve => resolve())
        }
    }
	visible = true;
...
// 	if(subPlaneta.orbitas.tipo === 4){
	...
	remove(item): void {
		...
		if (index >= 0) {
            ...
            this.servicio.desligar`+ planeta.singular + `(this.id, item.id).then(algo => {
	...
	selected(event: MatAutocompleteSelectedEvent): void {
		...
		if (!item_) {
            ...
            this.servicio.ligar`+ planeta.singular + `(this.id, item.id).then(ligado => {

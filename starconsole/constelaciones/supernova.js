...
module.exports = (constelacion, link, modulo, nivel) =>
    new Promise(resolve => {
        ...
        if(constelacion.status === 0){
            ...
            var escribirmodelo = () => {
                if(constelacion.tipo === 'ficha'|| constelacion.tipo === 'perfil'|| constelacion.tipo === 'vista'){
                    return escribir(basica + link + constelacion.nombre + '/' +  constelacion.nombre + '.module.ts')
                }else{
                    return null
                }
            }
